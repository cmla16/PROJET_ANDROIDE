...
model.setObjective(w[0]*sum(z2[e] for e in parcours) + w[1]*sum(z3[e] for e in parcours), GRB.MINIMIZE)
...
for e in parcours:
    ...
    if len(first) > 0:
        model.addConstr(
        z2[e] >= 1 - (1 / len(first)) * sum(x[e, u] for u in first),name=f"nb_ue_parcours_refusée_{e}")
