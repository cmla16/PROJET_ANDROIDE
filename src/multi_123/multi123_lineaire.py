...
def multi123_lineaire(path1, path2, path3, path4, path5, lambda1, lambda2, lambda3):
    ...
    model.setObjective(lambda1 * e1 + lambda2 * e2 + lambda3 * e3, GRB.MINIMIZE)
    ...
    if model.status == GRB.OPTIMAL:
        attributions("multi123_lineaire", x, y, parcours, ue_obligatoires, ue_preferences, groupes_td)
        stats("multi123_lineaire", parcours, z1, z2, z3)
